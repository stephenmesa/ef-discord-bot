  ...
  execute: (message) => {
    ...
    datastore.getLatestProgress(userId).then((latestProgress) => {
      ...
      datastore.getAllProgressEntriesForKL(kl, MAX_HISTORY_COUNT).then((progress) => {
        ...
        const messageText = `Your SR score is ${assessment.score}/100. (Your percentage is ${percentage.toFixed(2).toString()}% with an average percentage of ${assessment.percentageAverage.toFixed(2).toString()}% for KL${kl})`;
